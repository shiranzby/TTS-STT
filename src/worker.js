// ============================================================
// Shiran Voice — Cloudflare Worker Backend
// This file is the source. DO NOT EDIT index.js directly.
// Run `node build.js` to regenerate index.js from src/
// ============================================================
...
/* __HTML_PAGE__ will be injected by build.js */
...
async function getVoice(text, voiceName = "zh-CN-XiaoxiaoNeural", rate = '+0%', pitch = '+0Hz', volume = '+0%', style = "general", outputFormat = "audio-24khz-48kbitrate-mono-mp3") {
    try {
        ...
        const audioChunks = await processBatchedAudioChunks(
            chunks, voiceName, rate, pitch, volume, style, outputFormat,
            3, 800
        );
...
async function handleFileUpload(request) {
    try {
        ...
        if (!file.type.includes('text/') && !file.name.toLowerCase().endsWith('.txt') && !file.name.toLowerCase().endsWith('.md')) {
            return new Response(JSON.stringify({
                error: { message: "不支持的文件类型，请上传txt或md文件", type: "invalid_request_error", param: "file", code: "invalid_file_type" }
            }), { status: 400, headers: { "Content-Type": "application/json", ...makeCORSHeaders() } });
        }

        if (file.size > 100 * 1024 * 1024) {
            return new Response(JSON.stringify({
                error: { message: "文件大小超过限制（最大100MB）", type: "invalid_request_error", param: "file", code: "file_too_large" }
            }), { status: 400, headers: { "Content-Type": "application/json", ...makeCORSHeaders() } });
        }
        ...
        return await getVoice(
            ...
            numPitch >= 0 ? `+${numPitch}Hz` : `${numPitch}Hz`,
            numVolume >= 0 ? `+${numVolume}%` : `${numVolume}%`,
            style, "audio-24khz-48kbitrate-mono-mp3"
        );
...
async function handleAudioTranscription(request, env) {
    try {
        ...
        if (audioFile.size > 100 * 1024 * 1024) {
            return new Response(JSON.stringify({
                error: { message: "音频文件大小不能超过100MB", type: "invalid_request_error", param: "file", code: "file_too_large" }
            }), { status: 400, headers: { "Content-Type": "application/json", ...makeCORSHeaders() } });
        }
        ...
        // 优先使用环境变量中的 Token，其次是用户自定义，最后是硬编码默认值
        const defaultToken = env?.SILICONFLOW_TOKEN || 'sk-lqpenzrxbqhgehoymzgivldahdwslkenygijxkmzakrndewx';
